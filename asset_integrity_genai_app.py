...
with tabs[9]:
    st.subheader("📸 Visual Fault Describer")
    st.markdown("✅ Tab Loaded")

    uploaded_image = st.file_uploader("Upload equipment/fault image", type=["jpg", "jpeg", "png"])

    if uploaded_image:
        st.image(uploaded_image, caption="Uploaded Image", use_column_width=True)

        with st.spinner("Describing fault from uploaded image..."):
            prompt = f"Describe visible faults and risks based on the uploaded image: {uploaded_image.name}. Recommend actions."
            advisory = genai_advisory(prompt)
            if "⚠️" in advisory:
                st.error(advisory)
            else:
        ...
        st.info("Please upload an image to get a visual fault analysis.")
